...
class DataBase
{
    ...
    function addnewvisit($day , $time , $id , $isvideocall)
    {
        ...
        $this->sql ="INSERT INTO " . $table . " (patientid,day, time, isvideocall , verified) VALUES ('" . $id . "','" . $day . "','" . $time . "','" . $isvideocall . "','" . $verified . "')";
        if (mysqli_query($this->connect, $this->sql)) {
            return true;
        } else return false;
    }
    ...
    function checkVerification() 
    {
        ...
        $this->sql = $sql="select MAX(ind) from visitstimetable  WHERE
                            day ='" . $day . "'
                            and time <= '" . $time . "'";
        $result = mysqli_query($this->connect, $this->sql);
        $row = mysqli_fetch_assoc($result);
        if (mysqli_num_rows($result) != 0){
            $index  = $row['MAX(ind)'];
            $this->sql = $sql="select * from visitstimetable  WHERE
                            ind ='" . $index . "'";
    ...
    function checkEnd() 
    {
        ...
        $day = date("Y-m-d");
        $time =date('h:i:s');
        $this->sql = $sql="select MAX(ind) from visitstimetable visit WHERE
                            visit.day ='" . $day . "'
                            and visit.time <='" . $time . "'";
        $result = mysqli_query($this->connect, $this->sql);
        $row = mysqli_fetch_assoc($result);
        ...
        if (mysqli_num_rows($result) != 0){
            $index  = $row['MAX(ind)'];
            $this->sql = $sql="select * from visitstimetable visit WHERE
                            visit.ind ='" . $index . "'";
            $result = mysqli_query($this->connect, $this->sql);
            $row = mysqli_fetch_assoc($result);
            if (mysqli_num_rows($result) != 0) {
    ...
    function End() 
    {
        ...
        if (mysqli_num_rows($result) != 0){
            $index  = $row['MAX(ind)'];
            $this->sql = $sql="update visitstimetable  set end='" . $end . "' where 
                            ind ='" . $index . "'";
            if (mysqli_query($this->connect, $this->sql)) {
                    echo true;
            } else echo false;
